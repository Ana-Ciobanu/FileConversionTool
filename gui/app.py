...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.input_frame = ctk.CTkFrame(
            self.scroll, corner_radius=14, fg_color="#FFFFFF"
        )
        ...
        self.drop_label.grid(
            row=1, column=0, columnspan=2, sticky="ew", padx=14, pady=(0, 12)
        )
        ...
        self.selected_label.grid(
            row=2, column=0, columnspan=2, sticky="w", padx=14, pady=(0, 12)
        )
        ...
        self.btn_select_file.grid(
            row=3, column=0, sticky="ew", padx=(14, 7), pady=(0, 14)
        )
        ...
        self.btn_select_folder.grid(
            row=3, column=1, sticky="ew", padx=(7, 14), pady=(0, 14)
        )
        ...
        ctk.CTkLabel(
            self.output_frame, text="Output directory:", text_color="#374151"
        ...
        self.output_dir_entry.grid(
            row=2, column=1, sticky="ew", padx=(14, 300), pady=(0, 14)
        )
        ...
        self.btn_convert_file.grid(
            row=1, column=0, sticky="ew", padx=(14, 7), pady=(0, 10)
        )
        ...
        self.btn_convert_folder.grid(
            row=1, column=1, sticky="ew", padx=(7, 14), pady=(0, 10)
        )
        ...
        self.btn_extract_tables.grid(
            row=2, column=0, columnspan=2, sticky="ew", padx=14, pady=(0, 14)
        )

        # Status
        self.status_frame = ctk.CTkFrame(
            self.scroll, corner_radius=14, fg_color="#F7FAFF"
        )
    ...
    def _set_output_formats(self, formats: list[str]):
        ...
        # Exclude the input file's extension from the output formats
        input_ext = None
        if self.selected_file:
            input_ext = os.path.splitext(self.selected_file)[1].lower().lstrip(".")
        ...
        self.format_dropdown.configure(values=filtered_formats)

        if current in filtered_formats:
            self.format_dropdown.set(current)
        else:
            self.format_dropdown.set(filtered_formats[0])
    ...
    def _update_format_dropdown_for_selection(self):
        ...
        if self.selected_folder:
            ...
            formats = (
                sorted(set.intersection(*supported_sets)) if supported_sets else ["txt"]
            )
            self._set_output_formats(formats)
            return
    ...
    def _update_open_output_state(self):
        enabled = bool(
            self.last_generated_dir and os.path.isdir(self.last_generated_dir)
        )
    ...
    def open_output_dir(self):
        ...
        if not os.path.isdir(self.last_generated_dir):
            messagebox.showerror(
                "Open Output", "The generated output folder no longer exists."
            )
            self.last_generated_dir = None
            self._update_open_output_state()
            return
    ...
    def convert_file(self):
        ...
        def task():
            self.after(
                0,
                lambda: self._start_work(
                    f"Converting file to {output_format.upper()}..."
                ),
            )
            try:
    ...
    def convert_folder(self):
        ...
        output_format = self.format_dropdown.get().strip().lower()
        out_dir = self._resolve_output_dir() or os.path.join(
            self.selected_folder, "converted"
        )
        ...
        def task():
            self.after(
                0,
                lambda: self._start_work(
                    f"Converting folder to {output_format.upper()}..."
                ),
            )
            try:
                ...
                if results:
                    ...
                    self.after(
                        0,
                        lambda: messagebox.showinfo(
                            "No files", "No supported files were found in that folder."
                        ),
                    )
        ...
        def task():
            ...
            try:
                output_files = self.converter.extract_pdf_tables_to_csv(
                    pdf_path=self.selected_file, output_dir=out_dir
                )
                ...
                if output_files:
                    ...
                    self.after(
                        0,
                        lambda: messagebox.showinfo(
                            "No tables", "No tables were found in this PDF."
                        ),
                    )
    ...
    def _update_action_states(self):
        ...
        is_pdf = bool(
            self.selected_file and self.selected_file.lower().endswith(".pdf")
        )
